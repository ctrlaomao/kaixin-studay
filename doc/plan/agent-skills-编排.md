...
| S4 地基 | F00a / F00b / F00d / F00e / **F00g 入库** / F00f | 空壳 + 采集入云库 + P14 |
| F01–F28 | **每卡 1 子 agent** | 单一功能可验收 |
...
1. **产品：** 家庭自用微信小程序体验版，不正式上架；家长主操、学生答题。  
2. **知识点：** 只对照同步课堂 年级→学科→版本→册→章→课时；打星单位=课时。  
3. **不做：** 下载课件/视频/电子书、支付/商城、监考计时、班级老师端、语文开放批改。允许采集同步课堂公开目录元数据并按平台层级展示。  
4. **后端：** 微信云开发；密钥只在云函数；业务写库走云函数；**目录知识点存云数据库**。  
